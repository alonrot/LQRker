...
class BayesianLinearRegressionMarginalizedWeights(ABC):
	...
	This implementation is based on the derivations in [1].
	...
	The joint distribution of the weights and the noise variance is modeled
	using an prior inverse-normal-wishart distribution: p(w,v) ~ NIW(M,K,S0,N0). This
	class corresponds to the case in which w is marginalized out and v is kept fixed.
	If v is also marginalizing out, we'd need the T-matrix distribution, not implemented
	in this class (see [1, Sec.3])

	NOTE: The predictive distribution of this class is different from that of low-rank GPs.
	Also, this is not a Gaussian process, nor a reduced-rank GP, but just linear regression.
	...
	def __init__(self, in_dim, num_features, sigma2_n):
		...
		# Try different priors (none of them made a huge difference)
		# Diagonal matrix:
		self.K = tf.eye(self.num_features) # [Nfeat, Nfeat]
		
		# Diagonal matrix with weighted diagonal
		# self.K = tf.linalg.diag([0.01,0.1,1])

		# Transposition:
		# Kchol = tf.random.uniform(shape=(self.num_features,self.num_features),minval=0.0,maxval=1.0)
		# self.K = 0.5 * (Kchol @ tf.transpose(Kchol))
